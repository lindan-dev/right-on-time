...
function renderOlle(){
  ...
  for(let i=0;i<7;i++){
    ...
    if(olleEvs.length>0){inner+='<div class="olle-events">';olleEvs.forEach(ev=>{inner+='<div class="olle-event" onclick="say(\''+ev.name+(ev.time?' klockan '+ev.time:'')+'\')"><div class="olle-event-icon">'+ev.emoji+'</div><div><div class="olle-event-name">'+ev.name+'</div>'+(ev.time?'<div class="olle-event-time">kl '+ev.time+'</div>':'')+'</div><div class="olle-speak">🔊</div></div>';});inner+='</div>';}
